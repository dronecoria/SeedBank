...
void loop_display(void * parameter)
{
  ...
  for(;;) {
    ...
    if(state.avg_temperature == -9999){
      tft.fillScreen(ST77XX_GREEN);
      delay(500);
      continue;
    }

// if(state.temperatures.size()<3){
...
// }
    ...
    switch( state.getDisplayStatus() ){
      ...
      case DISPLAY_STATUS::INFO:
        ...
        tft.print(WiFi.SSID()); tft.print(" "); tft.print(WiFi.getHostname() ); tft.print("\n");
        tft.print(WiFi.networkID().toString() ); tft.print("\n");
        
        tft.setTextSize(2);
        tft.setTextColor(ST77XX_BLACK, ST77XX_YELLOW);
        tft.print(WiFi.localIP().toString() ); tft.print("\n");

        tft.print(WiFi.macAddress());tft.print("\n");
...
void display_time(Adafruit_ST7789 &tft){
  ...
  strftime (p, 80, "%p", &timeinfo);
    ...
    tft.print( h ); tft.print(":");tft.print( m ); tft.print(":"); tft.print( s ); tft.print(" "); tft.print( p ); tft.print("\n");
